            ...
            if st.button('⬇️ GET DATA'):
                ...
                if st.session_state.selected_site is not None:
                    ...
                    with tab3:
                        ...
                        def group_keywords(df, stop_words, min_group_size, ngram_size):
                            # Conteggio della frequenza di tutte le parole nelle keyword
                            all_words = list(itertools.chain(*df['Keyword'].str.lower().str.split()))
                            word_freq = Counter(all_words)
                        
                            # Seleziona solo le parole che sono comuni ma non troppo comuni (escludi le stop words e parole troppo corte)
                            common_terms = {word for word, freq in word_freq.items() if freq > 1 and not word in stop_words and len(word) > 2}
                        
                            # Crea una lista di DataFrame per i gruppi
                            grouped_dfs = []
                        
                            # Associa ogni keyword al termine più comune in essa contenuto
                            for keyword in df['Keyword']:
                                words = re.findall(r'\b\w+\b', keyword.lower())  # Estrai parole complete dalla keyword
                                if len(words) >= ngram_size:
                                    ngrams = [tuple(words[i:i + ngram_size]) for i in range(len(words) - ngram_size + 1)]
                                    groups = set()
                                    for ngram in ngrams:
                                        if all(term in common_terms or term.isdigit() for term in ngram):  # Considera anche numeri
                            ...
                            # Filtra i gruppi con una grandezza minima
                            filtered_groups = grouped_keywords_df.groupby('Group').filter(lambda x: len(x) >= min_group_size)
                        
                            return filtered_groups
                        
                        # Funzione per calcolare il totale dei click per ciascun gruppo
                        def calculate_click_totals(df, grouped_df):
                            click_totals = {}
                            for group in grouped_df['Group'].unique():
                                keywords_in_group = grouped_df[grouped_df['Group'] == group]['Keywords'].tolist()
                                click_total = df[df['Keyword'].isin(keywords_in_group)]['Clicks'].sum()
                        ...
                        # Controllo per la grandezza minima del gruppo
                        min_group_size = st.slider("Grandezza Minima del Gruppo", 1, 10, 2)
